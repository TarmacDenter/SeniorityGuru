          ...
          <div class="grid grid-cols-1 lg:grid-cols-4 gap-4 lg:gap-6">
            ...
            <template v-if="userFound">
              <DashboardTrajectoryChart
                :data="trajectoryData"
                class="lg:[grid-column:1/3] lg:[grid-row:span_2] dashboard-enter"
                style="animation-delay: 260ms"
              />
              <DashboardRetirementComparison
                :quals="quals"
                :compute-projection="computeRetirementProjection"
                class="lg:[grid-column:3/5] lg:[grid-row:span_2] dashboard-enter"
            ...
            <DashboardBaseStatusTable
              v-if="userFound"
              :data="baseStatusData"
              class="lg:[grid-column:1/3] dashboard-enter"
              style="animation-delay: 380ms"
            />
            <DashboardAggregateStatsGrid
              :data="aggregateStats"
              class="lg:[grid-column:3/5] dashboard-enter"
              :style="{ animationDelay: userFound ? '420ms' : '300ms' }"
            />
            <DashboardRecentListsTimeline
              :lists="recentLists"
              class="lg:[grid-column:3/5] dashboard-enter"
              :style="{ animationDelay: userFound ? '460ms' : '340ms' }"
            />
          </div>
        </template>
      </div>
