    ...
    <ul class="divide-y divide-(--ui-border)">
      <li v-for="list in lists" :key="list.id" class="flex items-center justify-between gap-3 py-2.5 first:pt-0 last:pb-0">
        <div class="flex items-center gap-3 min-w-0">
          <UIcon :name="list.icon" class="text-lg text-(--ui-text-muted) shrink-0" />
          <div class="min-w-0">
            <p class="text-sm font-medium text-(--ui-text-highlighted) truncate">{{ list.title }}</p>
            <p class="text-xs text-(--ui-text-muted)">{{ list.description }} - {{ list.date }}</p>
          </div>
        </div>
        <UButton
