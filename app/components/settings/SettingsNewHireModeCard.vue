...
function onToggle(value: boolean) {
  ...
  if (value) {
    toast.add({ title: 'New Hire Mode enabled', color: 'success' })
  } else {
    toast.add({ title: 'New Hire Mode disabled' })
  }
}

function onReset() {
  newHireMode.reset()
  toast.add({ title: 'New Hire Mode cleared', color: 'success' })
}

defineExpose({ onToggle, onReset })
</script>
  ...
  <UCard>
    <template #header>
      <div class="flex items-center justify-between">
        ...
        <USwitch
          :model-value="newHireMode.enabled.value"
          :disabled="newHireMode.realUserFound.value"
    ...
    <div v-else-if="newHireMode.enabled.value" class="space-y-4">
      ...
      <div class="flex justify-end pt-2 border-t border-default">
        <UButton color="neutral" variant="ghost" icon="i-lucide-x" @click="onReset">
          Clear and disable
        </UButton>
      </div>
    </div>
